...
const Transactions = () => {
  ...
  const getPriceRangeData = () => {
    ...
    return ranges.map(range => ({
      range: range.range,
      count: orders.filter(o => o.totalPrice >= range.min && o.totalPrice < range.max).length,
      fill: range.color
    }));
  };
...
const columns = [
    ...
    renderCell: (params) => (
      <Box display="flex" alignItems="center" gap={1}>
        <Avatar sx={{ width: 24, height: 24, fontSize: 12 }}>
          {(params.row?.shippingAddress?.fullName || 'N').charAt(0)}
        </Avatar>
  ...
  { 
    ...
    renderCell: (params) => {
      const addr = params.row?.shippingAddress;
      const fullAddress = addr ? `${addr.street}, ${addr.ward}, ${addr.district}, ${addr.city}` : 'N/A';
      return (
        <Box display="flex" alignItems="center" gap={1}>
    ...
    Quản lý Giao dịch
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        {statsCards.map((card, index) => (
          <Grid item xs={12} sm={6} md={3} key={index} sx={{ mb: 2, width: '20%' }}>
            <Card 
              sx={{ 
                height: '100%',
